...
class dwdforecast(threading.Thread):
    def __init__(self, myqueue):
        try:
            ...
            self.sandia_module = self.sandia_modules[
                self.mymodule]  # is "LG Electronics Inc. LG335E1C-A5" in sam-library-cec-modules-2019-03-05.csv
            ...
            if (self.DBOutput == 1):
                try:
                    self.db = mysql.connector.connect(user=self.DBUser, passwd=self.DBPassword, host=self.DBHost,
    ...
    def updatesingleRowinDB(self, cursor, tablename, TTT, Rad1h, FF, PPPP, mytimestamp, Rad1Energy, ACSim, DCSim,
                            CellTempSim, Rad1wh):
        sql = "UPDATE " + str(tablename) + " SET " + " Rad1h= " + str(Rad1h) + ", PPPP = " + str(PPPP) + ", FF= " + str(
            FF) + ", TTT= " + str(TTT) + ", Rad1Energy= " + str(Rad1Energy) + ", ACSim= " + str(
            ACSim) + ", DCSim = " + str(DCSim) + ", CellTempSim =" + str(CellTempSim) + ", Rad1wh =" + str(
            Rad1wh) + " WHERE mytimestamp= " + str(mytimestamp)
    ...
    try:
        def run(self):
            while not self.event.is_set():  # In case the main process wants to shut us down...
                ...
                logging.debug("%s %s %s", ",dwdforecast : -BEFORE  if- time comparison :", self.mynewtime,
                              self.lasttimecheck)
                if (self.mynewtime > self.lasttimecheck):
                    logging.debug("%s %s %s", ",dwdforecast : -in if- time comparison :", self.mynewtime,
                                  self.lasttimecheck)
                    ...
                    try:
                        time.sleep(
                            10)  # Assumption is - we see the file on the DWD server - but it has not yet been copied over
                        # Download the file from `url` and save it locally under `self.file_name`:
                        with urllib.request.urlopen(self.url) as self.response, open(self.file_name,
                                                                                     'wb') as self.out_file:
                            shutil.copyfileobj(self.response, self.out_file)
                            logging.debug("%s %s %s", ",subroutine dwdforecast shutil command execution : ",
                                          self.response, self.out_file)
                        ...
                        with zipfile.ZipFile(self.file_name, "r") as zip_ref:
                            ...
                            logging.debug("%s %s", ",dwdforecast : -Starting File extraction from DWD download :",
                                          Myzipfilename)
                            zip_ref.extractall(self.targetdir)
                        logging.debug("%s %s", ",dwdforecast : -File that I extracted is zipfile :", Myzipfilename)
                        time.sleep(5)  # not sure if this gets rid of the access problems
                    except Exception as MyException:
                        logging.error("%s %s", ",subroutine dwdforecast exception getting the data from server : ",
                                      MyException)
                        # =============================================================================
                    ...
                    #
                    self.ns = {'dwd': 'https://opendata.dwd.de/weather/lib/pointforecast_dwd_extension_V1_0.xsd',
                    ...
                    #
                    self.timestamps = self.root.findall(
                    ...
                    for j in timevalue:
                        ...
                        i = i+1
                    """
                    logging.debug("%s %s", ",subroutine dwdforecast Number of Timestamps in kml file is : ",
                                  len(self.timevalue))

                    for self.elem in self.tree.findall('./kml:Document/kml:Placemark',
                                                       self.ns):  # Position us at the Placemark
                    ...
                    if (self.PrintOutput == 1):
                        try:
                            ...
                            for j in range(self.rows):
                                if (
                                        self.indexcounter_addrows > 0):  # We are adding from the point onward - see self.indexcounter_addrows if check below
                                    # print ("counting indices", self.indexcounter_addrows)
                                    self.MyWeathervalues.update({'mydatetime': self.mosmixdata[0][
                                        j]})  # This is the following format: 2018-12-25T07:00:00.000Z
                                    self.MyWeathervalues.update({'myTZtimestamp': self.mosmixdata[1][
                                        j]})  # This is the following format: 2020-10-31 14:00:00.000
                    ...
                    try:
                        ...
                        self.mycolumns = {'mydatetime': np.array(self.mosmixdata[1]),
                                          'myTZtimestamp': np.array(self.mosmixdata[1]),
                                          'Rad1h': np.array(self.mosmixdata[2]), 'TTT': np.array(self.mosmixdata[3]),
                                          'PPPP': np.array(self.mosmixdata[4]), 'FF': np.array(self.mosmixdata[5])}
                        ...
                        self.PandasDF.Rad1h = self.PandasDF.Rad1h.astype(
                        ...
                        self.PandasDF[
                            'Rad1wh'] = 0.277778 * self.PandasDF.Rad1h  # Converting from KJ/m² to Wh/m² -and adding as new column Rad1wh
                        ...
                        logging.debug("%s %s", ",dwdforecast : -Starting PANDAS processing ... 12 ",
                                      len(self.local_timestamp))
                        ...
                        logging.debug("%s %s", ",dwdforecast : -Starting PANDAS processing ... 17 Dataframe length :",
                                      len(self.PandasDF.index))
                        ...
                        self.DNI = pvlib.irradiance.disc(ghi=self.PandasDF.Rad1wh, solar_zenith=self.solpos.zenith,
                                                         datetime_or_doy=self.local_timestamp,
                                                         pressure=self.PandasDF.PPPP, min_cos_zenith=0.9, max_zenith=80,
                                                         max_airmass=12)
                        ...
                        self.DHI = pvlib.irradiance.erbs(ghi=self.PandasDF.Rad1wh, zenith=self.solpos.zenith,
                                                         datetime_or_doy=self.local_timestamp, min_cos_zenith=0.9,
                                                         max_zenith=80)
                        ...
                        self.dataheader = {'ghi': self.PandasDF.Rad1wh, 'dni': self.DNI.dni, 'dhi': self.DHI.dhi,
                                           'temp_air': self.PandasDF.TTT, 'wind_speed': self.PandasDF.FF}
                        ...
                        self.myModelChain = ModelChain(self.mysolarsystem, self.mypvliblocation, aoi_model='no_loss',
                        ...
                        try:
                            ...
                            if (pvlib.__version__ == "0.9.4"):
                                ...
                                logging.error("%s %s", ",Version 0.7.2 of pvlib is no longer supported : ",
                                              pvlib.__version__)
                                break
                                # self.myModelChain.run_model(times=self.mc_weather.index, weather=self.mc_weather)
                            else:
                        ...
                        if (self.CSVOutput == 1):
                            try:
                                ...
                                self.mc_weatherANDPandasDF = pd.concat([self.mc_weather, self.PandasDF],
                                                                       axis=1).reindex(self.PandasDF.index)
                        ...
                        if (self.DBOutput == 1):
                            ...
                            self.Database_found_filetimestamp = self.checkTimestampExistence(self.cur, self.DBTable,
                                                                                             int(self.PandasDFFirstTimestamp))
                            logging.debug("%s %s",
                                          ",dwdforecast : -Starting database output - Database_found_filetimestamp is : ",
                                          self.Database_found_filetimestamp)
                            ...
                            try:
                                for index, row in self.PandasDF.iterrows():
                                    ...
                                    self.Database_found_filetimestamp = self.checkTimestampExistence(self.cur,
                                                                                                     self.DBTable,
                                                                                                     int(self.PandasDFFirstTimestamp))
                                    if (
                                            self.Database_found_filetimestamp == 0):  # Meaning we don´t have an entry for given timestamp in DB yet
                                        logging.debug("%s %s",
                                                      ",dwdforecast : -Starting database addsingleRow2DB -timestamp is : ",
                                                      row['mydatetime'])
                                        self.MyWeathervalues.update(
                                            {'mydatetime': row['mydatetime'], 'Rad1h': row['Rad1h'], 'TTT': row['TTT'],
                                             'PPPP': row['PPPP'], 'FF': row['FF'], 'Rad1wh': row['Rad1wh'],
                                             'Rad1Energy': row['Rad1Energy'], 'mytimestamp': row['mytimestamp'],
                                             'ACSim': row['ACSim'], 'CellTempSim': row['CellTempSim'],
                                             'DCSim': row['DCSim']})
                                        self.addsingleRow2DB(self.cur, self.DBTable, self.MyWeathervalues)
                                    if (
                                            self.Database_found_filetimestamp == 1):  # Meaning we found the timestamp and need to update the specific row with new values
                                        logging.debug("%s %s",
                                                      ",dwdforecast : -Starting database updatesingleRowinDB -timestamp is : ",
                                                      row['mydatetime'])
                                        self.MyWeathervalues.update(
                                            {'mydatetime': row['mydatetime'], 'Rad1h': row['Rad1h'], 'TTT': row['TTT'],
                                             'PPPP': row['PPPP'], 'FF': row['FF'], 'Rad1wh': row['Rad1wh'],
                                        ...
                                        self.updatesingleRowinDB(self.cur, self.DBTable, row['TTT'], row['Rad1h'],
                                                                 row['FF'], row['PPPP'], row['mytimestamp'],
